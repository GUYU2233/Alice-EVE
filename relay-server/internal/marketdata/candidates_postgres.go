...
func (r *PostgresRepository) SearchCandidates(ctx context.Context, q CandidateSearch) (CandidatePage, error) {
	...
	rows, err := r.pool.Query(ctx, candidateSearchSQL, q.RegionIDs, q.Budget, q.CargoM3, q.SalesTaxRate, q.BrokerRate, q.MinProfit, q.MinProfitRate, q.Limit+1, q.Offset)
	if err != nil {
...
WITH selected AS MATERIALIZED (
 ...
 SELECT l.region_id,l.location_id,l.system_id,l.type_id,l.best_ask,l.ask_depth,l.best_bid,l.bid_depth,
        s.published_at,v.volume_m3
 FROM selected s JOIN region_market_best_levels l ON l.batch_id=s.current_batch_id AND l.region_id=s.region_id
 JOIN type_volume v ON v.type_id=l.type_id
), asks AS MATERIALIZED (
 SELECT *,ROW_NUMBER() OVER(PARTITION BY type_id ORDER BY best_ask,region_id,location_id) rn
 FROM levels WHERE best_ask > 0
), bids AS MATERIALIZED (
 SELECT *,ROW_NUMBER() OVER(PARTITION BY type_id ORDER BY best_bid DESC,region_id,location_id) rn
 FROM levels WHERE best_bid IS NOT NULL
), sized AS (
 SELECT a.type_id,a.region_id,a.location_id,a.system_id,a.best_ask price,a.ask_depth depth,a.published_at,a.volume_m3,
  ...
  LEAST(a.ask_depth,b.bid_depth,FLOOR($2/a.best_ask)::bigint,FLOOR($3/a.volume_m3)::bigint) qty
 FROM asks a JOIN bids b USING(type_id) WHERE a.rn=1 AND b.rn=1
 AND (a.region_id,a.location_id)<>(b.region_id,b.location_id) AND b.best_bid>a.best_ask
), valued AS (
 SELECT *,price*qty capital,(sell_price-price)*qty gross,(sell_price*qty*$4+price*qty*$5) fees FROM sized WHERE qty>0
)
...
FROM valued WHERE gross-fees >= $6 AND (gross-fees)/capital >= $7
ORDER BY gross-fees DESC,type_id LIMIT $8 OFFSET $9`
