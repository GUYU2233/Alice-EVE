...
func TestScoreDeterministicAndExpiry(t *testing.T) {
	...
	if got := Score(events, now); got != 60 {
		t.Fatalf("got %d", got)
	}
	if Score(events, now) != Score(events, now) {
		t.Fatal("non deterministic")
	}
}
func TestInvalidConfidence(t *testing.T) {
	now := time.Now()
	if Score([]IntelEvent{{Source: "x", Confidence: 2, Threat: "critical", ExpiresAt: now.Add(time.Hour)}}, now) != 0 {
		t.Fatal("invalid event scored")
	}
}
